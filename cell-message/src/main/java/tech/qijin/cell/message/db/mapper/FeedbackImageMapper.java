package tech.qijin.cell.message.db.mapper;

import java.util.List;
import org.apache.ibatis.annotations.Delete;
import org.apache.ibatis.annotations.DeleteProvider;
import org.apache.ibatis.annotations.Insert;
import org.apache.ibatis.annotations.InsertProvider;
import org.apache.ibatis.annotations.Param;
import org.apache.ibatis.annotations.Result;
import org.apache.ibatis.annotations.Results;
import org.apache.ibatis.annotations.Select;
import org.apache.ibatis.annotations.SelectKey;
import org.apache.ibatis.annotations.SelectProvider;
import org.apache.ibatis.annotations.Update;
import org.apache.ibatis.annotations.UpdateProvider;
import org.apache.ibatis.type.JdbcType;
import tech.qijin.cell.message.db.model.FeedbackImage;
import tech.qijin.cell.message.db.model.FeedbackImageExample;

public interface FeedbackImageMapper {
    @SelectProvider(type=FeedbackImageSqlProvider.class, method="countByExample")
    long countByExample(FeedbackImageExample example);

    @DeleteProvider(type=FeedbackImageSqlProvider.class, method="deleteByExample")
    int deleteByExample(FeedbackImageExample example);

    @Delete({
        "delete from feedback_image",
        "where id = #{id,jdbcType=BIGINT}"
    })
    int deleteByPrimaryKey(Long id);

    @Insert({
        "insert into feedback_image (channel, feedback_id, ",
        "url, update_time, ",
        "create_time)",
        "values (#{channel,jdbcType=VARCHAR}, #{feedbackId,jdbcType=BIGINT}, ",
        "#{url,jdbcType=VARCHAR}, #{updateTime,jdbcType=TIMESTAMP}, ",
        "#{createTime,jdbcType=TIMESTAMP})"
    })
    @SelectKey(statement="SELECT LAST_INSERT_ID()", keyProperty="id", before=false, resultType=Long.class)
    int insert(FeedbackImage record);

    @InsertProvider(type=FeedbackImageSqlProvider.class, method="insertSelective")
    @SelectKey(statement="SELECT LAST_INSERT_ID()", keyProperty="id", before=false, resultType=Long.class)
    int insertSelective(FeedbackImage record);

    @SelectProvider(type=FeedbackImageSqlProvider.class, method="selectByExample")
    @Results({
        @Result(column="id", property="id", jdbcType=JdbcType.BIGINT, id=true),
        @Result(column="channel", property="channel", jdbcType=JdbcType.VARCHAR),
        @Result(column="feedback_id", property="feedbackId", jdbcType=JdbcType.BIGINT),
        @Result(column="url", property="url", jdbcType=JdbcType.VARCHAR),
        @Result(column="update_time", property="updateTime", jdbcType=JdbcType.TIMESTAMP),
        @Result(column="create_time", property="createTime", jdbcType=JdbcType.TIMESTAMP)
    })
    List<FeedbackImage> selectByExample(FeedbackImageExample example);

    @Select({
        "select",
        "id, channel, feedback_id, url, update_time, create_time",
        "from feedback_image",
        "where id = #{id,jdbcType=BIGINT}"
    })
    @Results({
        @Result(column="id", property="id", jdbcType=JdbcType.BIGINT, id=true),
        @Result(column="channel", property="channel", jdbcType=JdbcType.VARCHAR),
        @Result(column="feedback_id", property="feedbackId", jdbcType=JdbcType.BIGINT),
        @Result(column="url", property="url", jdbcType=JdbcType.VARCHAR),
        @Result(column="update_time", property="updateTime", jdbcType=JdbcType.TIMESTAMP),
        @Result(column="create_time", property="createTime", jdbcType=JdbcType.TIMESTAMP)
    })
    FeedbackImage selectByPrimaryKey(Long id);

    @UpdateProvider(type=FeedbackImageSqlProvider.class, method="updateByExampleSelective")
    int updateByExampleSelective(@Param("record") FeedbackImage record, @Param("example") FeedbackImageExample example);

    @UpdateProvider(type=FeedbackImageSqlProvider.class, method="updateByExample")
    int updateByExample(@Param("record") FeedbackImage record, @Param("example") FeedbackImageExample example);

    @UpdateProvider(type=FeedbackImageSqlProvider.class, method="updateByPrimaryKeySelective")
    int updateByPrimaryKeySelective(FeedbackImage record);

    @Update({
        "update feedback_image",
        "set channel = #{channel,jdbcType=VARCHAR},",
          "feedback_id = #{feedbackId,jdbcType=BIGINT},",
          "url = #{url,jdbcType=VARCHAR},",
          "update_time = #{updateTime,jdbcType=TIMESTAMP},",
          "create_time = #{createTime,jdbcType=TIMESTAMP}",
        "where id = #{id,jdbcType=BIGINT}"
    })
    int updateByPrimaryKey(FeedbackImage record);
}