...
public class CellFeedController {
    ...
    @PostMapping("/delete")
    public ResultVo deleteFeed(Long feedId) {
        if (cellFeedService.deleteFeed(feedId, UserUtil.getUserId())) {
            return ResultVo.instance().success();
        }
        return ResultVo.instance().fail(ResEnum.FAIL);
    }
    ...
    public ResultVo deleteComment(Long commentId) {
        if (cellCommentService.deleteComment(commentId)) {
            return ResultVo.instance().success();
        }
        return ResultVo.instance().fail(ResEnum.FAIL);
    }
